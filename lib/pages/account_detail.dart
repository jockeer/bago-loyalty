...
import 'package:bago/models/cuenta_puntos.dart';
...
import 'package:bago/providers/logout_provider.dart';
import 'package:bago/providers/providers.dart';
import 'package:bago/providers/puntos_provider.dart';
import 'package:bago/providers/user_update_provider.dart';
...
class _AccountDetailState extends State<AccountDetail> {
   ...
   TextEditingController controladorName, controladorApellido, controladorCorreo , controladorContrasena
          , controladorNroCarnet, controladorCiudadExpedicion, controladorGenero, controladorFechaNac,
          controladorCiudad, controladorPais,controladorCelular;
   Base base;
  ...
  void initState() {
    ...
    this.listaDeGeneros      = new List();
     ...
     new Future.delayed(Duration.zero,() {
             ...
             obtenerCuenta.obtenerCuentaPuntos(context)
                  .then((onValue){
                        Map<String, dynamic> cuentaRecibida = onValue;
                        if( cuentaRecibida[Constantes.estado] == Constantes.respuesta_estado_ok ){
                            ...
                            this.controladorGenero.text              = (this.cuenta.data.gender == 0 || this.cuenta.data.gender == "0") ? "Seleccione"
                            : ( (  this.cuenta.data.gender == 2 || this.cuenta.data.gender == "2"  ) ? "Femenino" : "Masculino"  );
                          

                            String fechaDesdeServe = this.cuenta.data.birthdate.toString().split(" ")[0];
 ...
 _fillStreamingsDefaults( BuildContext contexto){
   ...
   provedorDeBloc.addDataToStreamLastNameDetalleCuen( this.controladorApellido.text );
   provedorDeBloc.addDataToStreamEmailDetalleCuenta( this.controladorCorreo.text );
   provedorDeBloc.addDataToStreamGeneroDetalleCue( this.controladorGenero.text );
   provedorDeBloc.addDataToStreamFechaNacDetalleCuenta( this.controladorFechaNac.text );
 } 
         ...
         body: StreamBuilder<bool>(
           ...
           builder: (context, snapshot) {
             return ModalProgressHUD(
                  ...
                  inAsyncCall: ( snapshot.data != null && snapshot.data ) ? true : false ,
                  color: Colors.white,
                  dismissible: false,
        ...
        return SafeArea(
                ...
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: <Widget>[
                        ...
                        Text("Detalles de la cuenta", style: TextStyle( color: Colors.white),),
                        StreamBuilder<bool>(
                            ...
                            return GestureDetector(
                                    onTap: (){  
                                        if(provedorDeBloc.ultimoValorStreamEditingOrNot){
                                            ...
                                            FocusScope.of(contextoParteSuperior).requestFocus(new FocusNode());
                                          
                                            
                                                            _guardarDAtos(snapshot,context);
                                                      
                                        }
                                        provedorDeBloc.addDataToStreamEditingOrNot(!provedorDeBloc.ultimoValorStreamEditingOrNot);
                                    },
                                    child: Padding(
      ...
      if( asyncSnapshot.data != null &&  asyncSnapshot.data ){
          ...
          if( provedorDeBloc.ultimoValorGeneroDetalleCuen != null && 
              ...
              this.parametrosParaEnviar.addAll({
                    Constantes.GENDER   : (provedorDeBloc.ultimoValorGeneroDetalleCuen == "Femenino") ? "2" : "1"
              });
          }

          this.verificadorInternet.verificarConexion()
              .then((onValue){
                   
                    if( onValue[Constantes.estado] == Constantes.respuesta_estado_ok ){
                          provedorDeBloc.addDataToCargandoDetalleCuenta(true);
                          updateUserProvider.registerUser(this.parametrosParaEnviar, contexto)
                              .then((onValue){
                                    if( onValue[Constantes.estado] == Constantes.respuesta_estado_ok ){
                                            this.base.showSnackBar(onValue[Constantes.mensaje], contexto, Colores.COLOR_AZUL_ATC_FARMA);
                                            provedorDeBloc.addDataToCargandoDetalleCuenta(false);
                                    }else {
                                            this.base.showSnackBar("There was a problem", contexto, Colors.brown);
                                            provedorDeBloc.addDataToCargandoDetalleCuenta(false);
                                    }
                              });
                    }else {
                        this.base.showSnackBar(Constantes.error_conexion, contexto, Colors.brown);
                    }
              });
          ...
          this.base.showSnackBar("Llenar todos los campos correctamente", contexto, Colors.brown);
      }
  }
              ...
              child: FutureBuilder(
                    ...
                    builder: ( BuildContext contexto, AsyncSnapshot<Map<String, dynamic>> asyncSnapshot ){
                          if( asyncSnapshot.hasData && !asyncSnapshot.hasError ){
                                if( asyncSnapshot.data[Constantes.estado] == Constantes.respuesta_estado_ok ){
                                   this.cuenta = asyncSnapshot.data[Constantes.mensaje] as CuentaPuntos;
                                   print(this.cuenta);
                                    return Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: <Widget>[
                                          Text("INFORMACION PRINCIPAL", style: TextStyle(   color: Colors.white ),),
                                          ...
                                          SizedBox( height: 15.0, ),
                                          Text("INFORMACION ADICIONAL", style: TextStyle(   color: Colors.white ),),
                                          SizedBox( height: 15.0, ),
                                         ...
                                         SizedBox( height: 10.0, ),
                                          ...
                                          SizedBox( height: 15.0, ),
                                          Text("INFORMACION DE CONTACTO", style: TextStyle(   color: Colors.white ),),
                                          SizedBox( height: 15.0, ),
                                      ...
                                      return  Container(
                                        ...
                                        child: Center(
                                          child: Text(asyncSnapshot.data[Constantes.mensaje], style: TextStyle( color: Colors.white ),),
                                        ),
                                     );
                                }
                                ...
                                if( asyncSnapshot.hasError ){
                                      return  Container(
                                        height: MediaQuery.of(contexto).size.height * 0.9,
                                        child: Center(
                                          child: Text(asyncSnapshot.error.toString(), style: TextStyle( color: Colors.white ),),
                                        ),
                                     );
                                }
                              ...
                              return  Container(
                                        height: MediaQuery.of(contexto).size.height * 0.9,
                                        child: Center(
                                          child: this.base.retornarCircularCargando(Colores.COLOR_NARANJA_ATC_FARMA),
                                        ),
                                  );
                          }
                    }
          ...
          child: Column(
               ...
               children: <Widget>[
                      Padding(
                        ...
                        child: Image(image: AssetImage("assets/imagenes/imagenes_farmacorp/farmacorp_logo.png")),
                      ),
                      Padding(
                        padding: const EdgeInsets.all(0.0),
                        child: Text( "¿Estás seguro que deseas salir de la aplicación?", textAlign: TextAlign.center ,style: TextStyle( color: Colors.white, ), ),
                      ),
                                      ...
                                      .then((onValue){
                                          if( onValue[Constantes.estado] == Constantes.respuesta_estado_ok ){
                                              ...
                                              logoutProvider.logOutUser()
                                                    .then((onValue){
                                                          this.preferencias.agregarValor(Constantes.last_page, WelcomePage.nameOfPage);
                                                          Navigator.of(contextoFilaBotones).popAndPushNamed(WelcomePage.nameOfPage);
                                                    });
                                          }else {
                                              this.base.showSnackBar(Constantes.error_conexion, contextoFilaBotones, Colores.COLOR_AZUL_ATC_FARMA);
                                          }
                                      });
                            ...
                            return GestureDetector(
                                                  child:  Card(
                                                      ...
                                                      child: Padding(
                                                        ...
                                                        child: Center(child: Text(this.listaDeGeneros[indice])),
                                                      ) ,  
                                                  ),
                                                  onTap: (){
                                                        print(this.listaDeGeneros[indice]);
                                                        this.controladorGenero.text = this.listaDeGeneros[indice];
                                                       provedorDeBloc.addDataToStreamGeneroDetalleCue(this.listaDeGeneros[indice]);
           ...
           return TextField(
                                              ...
                                              onChanged: ( value ){
                                               ...
                                               onTap: ( !snapshot.hasData || !snapshot.data ) ? () {
                                                       ...
                                                       FocusScope.of(contextoFecha).requestFocus(new FocusNode());
          ...
          builder: ( BuildContext contexto , Widget child ) {
              return Theme(
                  data: ThemeData.light().copyWith(
                    ...
                    colorScheme: ColorScheme.light( primary  : Colores.COLOR_ROJISO_TEXTO_LOYALTY ),    
                    primaryColor: Color.fromRGBO(102, 10, 11, 1.0),//Head background
                    accentColor: Color.fromRGBO(102, 10, 11, 1.0)//selection color
          ...
          return TextField(
                                              ...
                                              onChanged: ( value ){
                                               ...
                                               onTap: ( !snapshot.hasData || !snapshot.data ) ? () {
                                                                ...
                                                                FocusScope.of(contextoGenero).requestFocus(new FocusNode());
               ...
               return TextField(
                                          ...
                                          onChanged: ( value ){
                                           ...
                                           onTap: () {
                                              ...
                                              Navigator.of(contextoContrasena).popAndPushNamed(ChangePasswordPage.nameOfPage);
                                           } 
                                    );
                     ...
                     return StreamBuilder<bool>(
                          ...
                          builder: (context, snapshot) {
                            return TextField(
                                                            ...
                                                            onTap: ( !snapshot.hasData || !snapshot.data ) ? () {
                                                                FocusScope.of(contextoCorreo).requestFocus(new FocusNode());
                                                            } : null
                                                      );
                          }
                        );
          ...
          return StreamBuilder<bool>(
                  ...
                  builder: (context, snapshot) {
                    return TextField(
                                                    ...
                                                    onTap: ( !snapshot.hasData || !snapshot.data ) ? () {
                                                        FocusScope.of(contextoApellido).requestFocus(new FocusNode());
                                                    } : null
                                              );
                  }
                );
                      ...
                      return TextField(
                                     ...
                                     decoration: InputDecoration(
                                        ...
                                        onChanged: ( value ){
                                         ...
                                         onTap: ( !snapshot.hasData || !snapshot.data ) ? () {
                                                 FocusScope.of(contextoTextName).requestFocus(new FocusNode());
                                              } : null
                                    );
                    }
                  );
